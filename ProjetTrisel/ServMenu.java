...
/**
 * Servlet implementation class ServMenu
 * gère toute l'application depuis la servlet, traitement, accès au modèle, affichage
 * un peu fouilli mais c'est possible
 */
@WebServlet("/ServMenu")
public class ServMenu extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		out.println("<form name = 'authentification' method='post' onSubmit='return verifForm(this)' action='./ServMenu'>");
		out.println("<table >");
		out.println("<tr>");
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		Usager usag = AccesData.getUsagerlogMdp(user, mdp);
		if (usag==null) {
			...
			if (listeHabitation.size() == 0 ) {
				...
				for (Habitation  h: listeHabitation)
				{
					...
					out.println(h.getCodePostal() + "  "+ h.getAdresseVille()+ "<br/>");
